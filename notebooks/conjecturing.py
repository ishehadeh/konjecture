...
class Conjecture(SageObject):  # Based on GraphExpression from IndependenceNumberProject
    ...
    def evaluate(self, g, returnBoundValue=False):
        stack = []
        if returnBoundValue:
            assert self.stack[-1][0] in {
                operator.le,
                operator.lt,
                operator.ge,
                operator.gt,
            }, "Conjecture is not a bound"
        for op, opType in self.stack:
            if opType == 0:
                ...
                if op == operator.truediv and right == 0:
                ...
                elif op == operator.pow and (
                    right == Infinity or right == float("inf")
                ):
                    if left < -1 or left > 1:
                        stack.append(float("inf"))
                    elif -1 < left < 1:
                    ...
                    op == operator.pow and left == -Infinity and right not in ZZ
                ):  # mimic C function pow
                    stack.append(float("inf"))
                elif (
                    op == operator.pow and left < 0 and right not in ZZ
                ):  # mimic C function pow
                    stack.append(float("nan"))
...
def _makeConjecture(inputList, variable, invariantsDict):
    ...
    for op in inputList:
        if op in invariantsDict:
            ...
            if type(invariantsDict) is list:
                f = lambda obj: obj
            elif type(invariantsDict[op]) in (
            ...
            raise ValueError(
                "Error while reading output from expressions. Unknown element: {}".format(
                    op
                )
            )

    return Conjecture(
        operatorStack, expressionStack.pop(), (inputList, variable, invariantsDict)
    )
...
def _makePropertyBasedConjecture(inputList, invariantsDict):
    ...
    return PropertyBasedConjecture(
        propcalc.formula(expressionStack.pop()),
        propertyCalculators,
        (inputList, invariantsDict),
    )
...
def propertyBasedConjecture(
    ...
    command = command.format(
        ...
        "--all-operators " if operators is None else "",
        time,
        "--sufficient" if sufficient else "--necessary",
    )
...
def dfConjecture(
    ...
    # call the conjecturing program
    command = "../conjecturing/c/build/expressions-64 -c{}{} --dalmatian {}--time {} --invariant-names --output stack {} --allowed-skips 0"
    command = command.format(
        "v" if verbose and debug else "",
        "t" if theory is not None else "",
        "--all-operators " if operators is None else "",
    ...
    stdin = sp.stdin
    if stdin is None:
        print("subprocess stdin was None!")
        return
    if sp.stderr is None:
        print("subprocess stderr was None!")
        return
    try:
        if operators is not None:
            stdin.write("{}\n".format(len(operators)))
        ...
        stdin.write("{} {} {}\n".format(len(df), len(invariants), mainInvariant + 1))
        ...
        if theory is not None:
            ...
            bound = None
            if upperBound:
                bound = df.select(pl.min_horizontal(*theory).alias("bound"))
            else:
                bound = df.select(pl.max_horizontal(*theory).alias("bound"))
            for obj_bound in bound["bound"]:
                if upperBound:
                    stdin.write("{}\n".format(obj_bound))
        ...
        for row in df.rows(named=True):
            for invariant in invariants:
                stdin.write("{}\n".format(row[invariant]))
        stdin.flush()
    except BrokenPipeError:
        raise RuntimeError("\n".join(line for line in sp.stderr))
    ...
    for l in out:
        ...
        if op:
            ...
            conjectures.append(_makeConjecture(inputList, variable, invariants))
            inputList = []
